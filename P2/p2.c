...
int main(int argc, const char * argv[]) {
    ...
    //Make a copy of the board (used to output in the file later on)
    int origBoard[row][col];
    int z,y;
    for(z = 0; z < row; z++){
        for(y = 0; y < col; y++){
            origBoard[z][y] = board[z][y];
        }
    }
    ...
    int rowChoice2;
    int colChoice2;
    int flag;   //used to prevent CPU from memorizing the same card twice
    ...
    while(gameStillGoing == 1){
        ...
        for(i = 0; i < numCardsCanMemorize; i++){
            if(memory[1][i] == rowChoice1 && memory[2][i] == colChoice1 && index != numCardsCanMemorize){
                flag = 1;
            }
        }
        if(flag != 1){
            //We can safely remember this card (because there is room, and we haven't already done so
        ...
        for(i = 0; i < numCardsCanMemorize; i++){
            if(memory[1][i] == rowChoice2 && memory[2][i] == colChoice2 && index != numCardsCanMemorize){
                flag = 1;   //this means that somewhere (doesn't matter where), the CPU has already memorized that specific card
            }
        }
        if(flag != 1){
            //We can safely remember this card (because there is room, and we haven't already done so
        ...
        if(didMatch(board[rowChoice1][colChoice1], board[rowChoice2][colChoice2]) == 1){
            ...
            for(z = 0; z < numCardsCanMemorize; z++){
                if(memory[0][z] == board[rowChoice1][colChoice1]){
                    for(j = z; j < numCardsCanMemorize-1;j++){
                        memory[0][j] = memory[0][j+1];  //move everything forward by one to "forget" the matched cards
                        memory[1][j] = memory[1][j+1];
                        memory[2][j] = memory[2][j+1];
                        memory[0][numCardsCanMemorize-1] = -1;
                        memory[1][numCardsCanMemorize-1] = -1;
                        memory[2][numCardsCanMemorize-1] = -1;
                    }
                }
            }
        ...
        if(cardsLeft == 0){
            gameStillGoing = 0;
            writeToOutput(output2,roundNum, rowChoice1, colChoice1, rowChoice2, colChoice2, origBoard[rowChoice1][colChoice1], origBoard[rowChoice2][colChoice2], -1, -1,-1, -1, -1,-1);
        }
        /*************************************************************************/
        
        //Now, the computer moves (if there are cards left...)
        if(cardsLeft > 0){
            ...
            int j;
            for(j = 0; j < numCardsCanMemorize; j++){
                if(didCPUMatch(board[CPU_RowChoice1][CPU_ColChoice1], memory[0][j] == 1) && turnIsOver == 0){
                    //we've got a match - flip it and everything immediately
                    CPU_RowChoice2 = memory[1][j];
                    CPU_ColChoice2 = memory[2][j];
            ...
            if(noMatches == 1){
                ...
                for(m = 0; m < numCardsCanMemorize; m++){
                    while(board[CPU_RowChoice2][CPU_ColChoice2] == -1 || (CPU_RowChoice2 == CPU_RowChoice1 && CPU_ColChoice2 == CPU_ColChoice1)) {
                        //This card has already been removed/picked, pick another
                        CPU_RowChoice2 = rand() % row;
                        CPU_ColChoice2 = rand() % col;
                    }
                }
            ...
            //output to file
            writeToOutput(output2,roundNum, rowChoice1, colChoice1, rowChoice2, colChoice2, origBoard[rowChoice1][colChoice1], origBoard[rowChoice2][colChoice2], CPU_RowChoice1, CPU_ColChoice1,CPU_RowChoice2, CPU_ColChoice2, origBoard[CPU_RowChoice1][CPU_ColChoice1],origBoard[CPU_RowChoice2][CPU_ColChoice2]);
